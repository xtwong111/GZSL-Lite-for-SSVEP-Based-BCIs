...
class MainLite():
    ...
    def trainModel(self, model_name):
        ...
        vali_loader = torch.utils.data.DataLoader(TestDatasets(dataset_v), drop_last=True, **test_kwargs)
        test_all_loader = torch.utils.data.DataLoader(TestDatasets(dataset_t), drop_last=True, **test_kwargs)
        ...
        params = {
            ...
            # "freq_seg": args.freq_seg,
            "inter_ch": args.inter_channels,
            "device": device,
        }
        torch.cuda.set_device(device_ind)
...
class TrainDatasets(Dataset):
    def __init__(self, inargs):
        ...
        harmos_2 = self.sine_template.shape[1]
        cls_num = self.sine_template.shape[2] 
        Ns = self.sine_template.shape[3]
...
class GLite(nn.Module):
    def __init__(self, params):
        ...
        for sub_band_ind in range(self.num_sub_band):
            self.SB_ES.append(SineEmbedding(params))
            self.SB_ET.append(TemplateEmbedding(params))
            self.SB_EK.append(KxEmbedding(params))
            self.SB_EV.append(VxEmbedding(params))
            
            self.SB_WS1.append(ProjLayer(self.inter_channels,
                         self.cls_num))
            self.SB_WT1.append(ProjLayer(self.inter_channels,
                         self.cls_num))
            self.SB_WK.append(ProjLayer(self.inter_channels,
                         1))
            self.SB_WV.append(ProjLayer(self.inter_channels,
                         1))
            
            self.SB_WS2.append(ProjLayer(self.inter_channels,
                         self.cls_num))
            self.SB_WT2.append(ProjLayer(self.inter_channels,
                         self.cls_num))
            self.SB_WSX.append(ProjLayer(self.inter_channels,
                         self.cls_num))
            self.SB_WTX.append(ProjLayer(self.inter_channels,
                         self.cls_num))
            
            self.SB_ST.append(STLayer(self.inter_channels,
                         self.cls_num))
            
            self.SB_WOsx.append(OutpLayer(self.inter_channels))
            self.SB_WOtx.append(OutpLayer(self.inter_channels))
    ...
    def forward(self, X, S, T, batch_idx=0, epoch=0):
        ...
        for sub_band_ind in range(self.num_sub_band):
            x = X[:, sub_band_ind, :, :].unsqueeze(2)
            t = T[:, sub_band_ind, :, :, :]
            s = S[:, :, :, :]
            
            E_S = self.SB_ES[sub_band_ind](s, batch_idx, epoch)
            E_S = E_S[:, :, :, self.sine_latence_window:]
            E_T = self.SB_ET[sub_band_ind](t, batch_idx, epoch)
            E_K = self.SB_EK[sub_band_ind](x, batch_idx, epoch)
            E_V = self.SB_EV[sub_band_ind](x, batch_idx, epoch)
            
            Q_S1 = self.SB_WS1[sub_band_ind](E_S)
            Q_T1 = self.SB_WT1[sub_band_ind](E_T)
            K = self.SB_WK[sub_band_ind](E_K)
            V = self.SB_WV[sub_band_ind](E_V)
            ...
            Q_S2 = self.SB_WS2[sub_band_ind](E_S)
            Q_T2 = self.SB_WT2[sub_band_ind](E_T)
            SX = self.SB_WSX[sub_band_ind](Att_SX1) 
            TX = self.SB_WTX[sub_band_ind](Att_TX1) 
            ...
            C1[:,sub_band_ind,:] = self.SB_WOsx[sub_band_ind](Que_SX2).squeeze()
            C2[:,sub_band_ind,:] = self.SB_WOtx[sub_band_ind](Que_TX2).squeeze()
            C3[:,sub_band_ind,:] = self.SB_ST[sub_band_ind](E_S, E_T).squeeze()
